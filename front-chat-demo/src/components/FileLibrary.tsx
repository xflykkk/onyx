...
const FolderList: React.FC<FolderListProps> = ({
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      ...
      {/* 文件夹列表 */}
      <div className="flex-1 space-y-2 overflow-y-auto min-h-0 max-h-[20rem]">
        {/* 全部文件选项 */}
        <motion.button
        ...
        <AnimatePresence>
          {filteredFolders.map((folder) => (
            <motion.div
              ...
              layout
              className={cn(
                'flex items-center space-x-2 px-2 py-1.5 rounded-lg border text-left transition-all group cursor-pointer',
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <label 
            ...
            title={!selectedFolderId ? '请先选择一个文件夹' : '上传文件夹'}
          >
            <Upload className="w-6 h-6" />
            <span className="text-xs hidden sm:inline">
              {!selectedFolderId ? '选择文件夹' : '上传文件夹'}
            </span>
            <input
              type="file"
              multiple
      ...
      >
        {!selectedFolderId ? (
          ...
          <>
            ...
            <p className="font-medium">拖拽文件夹到这里或点击上传</p>
            <p className="text-sm opacity-70">支持文件夹批量上传</p>
          </>
        )}
      </div>
      ...
      <div className="space-y-2 max-h-[20rem] overflow-y-auto">
        ...
        {filteredFiles.length === 0 && selectedFolderId && !searchTerm && (
          <div className={cn(
            ...
            <p>这个文件夹还没有文件</p>
            <p className="text-sm opacity-70">拖拽文件夹或点击上传按钮添加文件</p>
          </div>
        )}
...
export const FileLibrary: React.FC<FileLibraryProps> = ({
  ...
  return (
    <div className={cn('max-h-[32rem] flex flex-col lg:grid lg:grid-cols-2 gap-6', className)}>
      {/* 文件夹列表 */}
      <div className={cn(
        'p-4 rounded-xl border flex flex-col min-h-0 max-h-[30rem]',
        variant === 'glassmorphism'
