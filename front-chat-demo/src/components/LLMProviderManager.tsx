...
export default function LLMProviderManager({ variant = 'glassmorphism' }: LLMProviderManagerProps) {
  ...
  const [loadingSearchSettings, setLoadingSearchSettings] = useState(false);
  ...
  // Track original API key to detect actual changes
  ...
  const loadSearchSettings = async () => {
    try {
      setLoadingSearchSettings(true);
      const settings = await getCurrentSearchSettings();
      setSearchSettings(settings);
      setSelectedLanguages(settings.multilingual_expansion || []);
    } catch (err: any) {
      console.error('Failed to load search settings:', err);
    } finally {
      setLoadingSearchSettings(false);
    }
  };
  ...
  const handleSaveLanguageSettings = async () => {
    try {
      ...
      const request: UpdateSearchSettingsRequest = {
        multilingual_expansion: selectedLanguages,
      };
      
      await updateSearchSettings(request);
      await loadSearchSettings();
      
      setError('✅ Language settings updated successfully!');
  ...
  const handleUpdateProvider = async () => {
    try {
      ...
      const apiKeyActuallyChanged = formData.api_key !== originalApiKey;
      ...
      const request: CreateLLMProviderRequest = {
        ...
        default_vision_model: formData.default_vision_model,
        model_configurations: modelConfigs,
        api_key_changed: apiKeyActuallyChanged, // Only set true if API key actually changed
      };
  ...
  const handleEditProvider = (provider: LLMProvider) => {
    ...
    const originalKey = provider.api_key || '';
    setOriginalApiKey(originalKey);
    setFormData({
      name: provider.name,
      provider: provider.provider,
      api_key: originalKey,
      ...
      api_key_changed: false, // Don't assume API key is changed when editing
    });
  ...
  const resetForm = () => {
    ...
    setOriginalApiKey(''); // Reset original API key tracking
  };
    ...
    <div className="space-y-6">
      ...
      <div className={getCardClass()}>
        <div className="space-y-4">
          ...
          <p className={cn('text-sm', getSubTextClass())}>
            Configure language expansion for search queries. Select one or more languages to enable multilingual search.
          </p>

          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              {['Chinese', 'English', 'Spanish', 'French', 'German', 'Japanese', 'Korean', 'Portuguese'].map((language) => (
                <label
                  key={language}
                  className={cn(
            ...
            <div className={cn(
              ...
              <p>
                <strong>Currently selected:</strong> {selectedLanguages.length === 0 ? 'None (single language mode)' : selectedLanguages.join(', ')}
              </p>
            </div>
            ...
            <div className="flex justify-end">
              <motion.button
                ...
                className={cn(
                  ...
                  savingSearchSettings && 'opacity-50 cursor-not-allowed'
                )}
              >
                {savingSearchSettings ? (
                  <>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setFormData(prev => ({ 
                        ...prev, 
                        api_key: newApiKey,
                        api_key_changed: editingProvider ? (newApiKey !== originalApiKey) : true
                      }));
                    }}
                    className={cn(
                      'w-full p-3 rounded-lg border',
